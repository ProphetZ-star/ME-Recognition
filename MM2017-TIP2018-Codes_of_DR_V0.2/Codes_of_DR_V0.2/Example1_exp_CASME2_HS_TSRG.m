% This is an example of using TSRG (DRFS-S) method to deal with the cross-database micro-expression recognition
...
for lambda = [0.001,0.01,0.1,1,10,100,1000]
    for mu = [0.001:0.001:0.009,0.01:0.01:0.09,0.1:0.1:1,2:10]
        ...
        % --------------training TSRG (DRFS-S)-------------------
        [P,Ks,Kt] = IALM_TSRG(Y_s,Y_te,options,lambda,mu);
        
        % --------------regenerating target data -------------------
        Regenerated_Y_te = P'*Kt;

        % --------------training SVM and predicting target data-------------------
        linear_model = svmtrain(X_s_label, Y_s', '-t 0');
